...
function MusicPlayerContent() {
  const { user }: { user: LocalUser | null } = useAuth();
  const {
    songs,
    trendingSongs,
    playlists,
  ...
  } = useSupabaseData(user as LocalUser | null);
  ...
  const loadLastPlayedImage = async () => {
    if (user && lastPlayedSong && !hasSetLastPlayedSong && !lastPlayedSongDismissed) {
      try {
        ...
        const initialRecs = await getPersonalizedSongs(user.email, lastPlayedSong, listenedSongs);
...
const handleSongPlay = async (song: Song) => {
  ...
  if (user) {
    try {
      ...
      const recs = await getPersonalizedSongs(user.email, song, listenedSongs);
...
const handleNext = async () => {
  ...
  if (nextQueueSong) {
    ...
    if (user) {
      const recs = await getPersonalizedSongs(user.email, nextQueueSong, listenedSongs);
  ...
  if (currentSongIndex === personalizedList.length - 2 && !isFetchingMore && user && currentSong) {
    ...
    try {
      ...
      if (currentBatchListenedSongs.length > 0) {
        ...
        newRecs = await getPersonalizedSongs(user.email, currentSong, listenedSongs);
      }
